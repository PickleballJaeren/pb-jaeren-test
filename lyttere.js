...
export function startSkjermSyncLytter(treningId, callbacks = {}) {
  ...
  // Les eksisterende ts fra Firestore-dokumentet først.
  // Setter sisteTs til eksisterende verdi slik at gamle hendelser ignoreres.
  let sisteTs = Date.now(); // fallback hvis dokument ikke finnes ennå
  let lytterStartet = false;

  getDoc(doc(db, SAM.SKJERMSYNC, treningId))
    .then(snap => {
      if (snap.exists()) {
        const eksisterendeTs = snap.data()?.ts ?? 0;
        // Bruk den høyeste verdien — beskytter mot klokkeskjevhet mellom klienter
        sisteTs = Math.max(sisteTs, eksisterendeTs);
      }
    })
    .catch(() => {})
    .finally(() => {
      if (lytterStartet) return; // allerede startet av onSnapshot
      startLytter();
    });

  function startLytter() {
    lytterStartet = true;
  ...
  // Start lytteren umiddelbart — getDoc og onSnapshot kjører parallelt.
  // onSnapshot gir cached data med en gang, getDoc gir fersk ts.
  startLytter();
}
