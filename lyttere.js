...
export function startLyttere(callbacks = {}) {
  ...
  const l1 = onSnapshot(
    ...
    snap => {
      ...
      // Admin viser resultater — synkroniser alle til resultat-skjermen.
      // Sjekk lokalt flagg som settes av admin ved rundebytte for å unngå re-trigger.
      if (data.visResultat === true && !app._ignorerVisResultat && forrigeRunde !== 0 && app.runde === forrigeRunde) {
        callbacks.onVisRundeResultat?.();
      }
      // Nullstill flagget etter første snapshot
      app._ignorerVisResultat = false;
    },
...
// skjermSync er fjernet — synkronisering skjer via treningsdokumentet
export function startSkjermSyncLytter() {}
