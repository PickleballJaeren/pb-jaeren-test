...
export function startLyttere(callbacks = {}) {
  ...
  const l1 = onSnapshot(
    ...
    snap => {
      ...
      // Admin har satt skjermStatus — naviger alle dit.
      // Bruker skjermStatusTs (timestamp) for å fange opp gjentatte navigeringer
      // til samme skjerm, siden Firestore bare trigger onSnapshot ved faktisk endring.
      const skjermStatus   = data.skjermStatus;
      const skjermStatusTs = data.skjermStatusTs?.toMillis?.() ?? 0;
      const forrigeTs      = app._skjermStatusTs ?? 0;

      if (skjermStatus && skjermStatusTs > forrigeTs) {
        app._skjermStatusTs = skjermStatusTs;
        if (skjermStatus === 'slutt') {
          callbacks.onVisResultater?.();
          return;
        }
        if (skjermStatus === 'resultat') {
