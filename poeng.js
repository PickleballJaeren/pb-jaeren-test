...
export function validerInndata(i, endretFelt) {
  ...
  if (autofylte) {
    const antallKamper = hentParter(bane, erMix(), app.er6SpillerFormat).length;
    setTimeout(() => {
...
export async function autolagreKamp(i, l1, l2) {
  ...
  try {
    ...
    if (!erSingelBane(bane) && par.hviler != null && bane?.spillere?.[par.hviler]) {
    ...
    // Naviger automatisk tilbake til baneoversikten etter kort pause
    // slik at brukeren ser "✓ Lagret" før skjermen bytter
    setTimeout(() => {
      if (typeof window.naviger === 'function') window.naviger('baner');
    }, 600);
  } catch (e) {
