...
function kryssSeeding3Puljer(meta, puljer) {
  ...
  const t1 = tredjepl[0]; // beste 3.plass
  const t2 = tredjepl[1]; // nest beste 3.plass

  // Alle tre vinnere møter 3.plassene, Nr.2 møter Nr.2
  // QF1: V_A vs t1, QF2: V_B vs t2, QF3: V_C vs Nr2_A, QF4: Nr2_B vs Nr2_C
  let par = [
    _velg(pA, 1), t1?.lagId ?? null,  // QF1
    _velg(pB, 1), t2?.lagId ?? null,  // QF2
    _velg(pC, 1), _velg(pA, 2),       // QF3
    _velg(pB, 2), _velg(pC, 2),       // QF4
  ];
