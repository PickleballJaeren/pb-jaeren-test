...
export function validerResultat(p1, p2, format) {
  // Slå sammen med STANDARD_KAMPFORMAT for å fylle ut manglende felt
  const f = { ...STANDARD_KAMPFORMAT, ...(format ?? {}) };
  const { points_to_win, win_by, max_points, type } = f;
  ...
  if (type === 'single') {
    ...
    // Cap-situasjon: begge hadde max_points-1 (f.eks. 14–14)
    const erCapSituasjon = taper === max_points - 1;
    if (erCapSituasjon) {
      if (vinner !== max_points)
    ...
    // Normalt: må vinne med minst win_by poeng
    if (diff < win_by)
...
function kryssSeeding3Puljer(meta, puljer) {
  // Vinnere: rang 1 fra A, B, C
  // Nr.2: rang 2 fra A, B, C
  // 3.plasser: rang 3 (sortert globalt)
  const grp       = _grupperPerPulje(meta);
  const [pA, pB, pC] = [grp[0] ?? [], grp[1] ?? [], grp[2] ?? []];
  const tredjepl  = meta.filter(l => l.puljeRang === 3).sort(sammenlignPaaTvers);
  const t1 = tredjepl[0];
  const t2 = tredjepl[1];

  // Oppsett: V_A vs t1, V_B vs Nr2_C, V_C vs Nr2_A, Nr2_B vs t2
  let par = [
    _velg(pA, 1), t1?.lagId ?? null,  // QF1
    _velg(pB, 1), _velg(pC, 2),       // QF2
    _velg(pC, 1), _velg(pA, 2),       // QF3
    _velg(pB, 2), t2?.lagId ?? null,  // QF4
  ];

  // Juster om vinner møter lag fra egen pulje
  par = _justerKryss3(par, meta, tredjepl, puljer);
  return par;
}
...
export async function startSluttspill(turneringId) {
  ...
  // Seed A-laget
  const seededeA = seedALag(kval.A, t.puljer, t.konfig?.seedingModus ?? SEEDING_MODUS.STANDARD, kval);
