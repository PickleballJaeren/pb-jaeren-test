...
export function validerResultat(p1, p2, format) {
  ...
  // Deuce-situasjon: taper har nådd points_to_win-1 eller mer (f.eks. 10–10, 11–11, 12–12)
  // Vinner må ha nøyaktig win_by poeng mer
  ...
  // Normalt spill: taper er under points_to_win-1
  // Vinner må ha nøyaktig points_to_win
  if (vinner !== points_to_win)
...
export function beregnEndeligRangering(turnering) {
  ...
  // Bygg lagMap med både id og lagId som nøkkel for robusthet
  const lagMap = {};
  for (const l of (lag ?? [])) {
...
function _hentVinnerFraKamp(kamper, kampId, plass, lagMap, ut) {
  ...
  if (!k?.ferdig) return;
  // Prøv vinnerId/winnerId først, fall tilbake på poeng
...
// ════════════════════════════════════════════════════════
// NULLSTILL NEDSTRØMS KAMPER (ved redigering)
// ════════════════════════════════════════════════════════
export async function nullstillNedstrømsKamper(turneringId, nivaa, kampId) {
  ...
  // Samle alle nedstrøms kamp-IDer rekursivt
  ...
  // Nullstill alle nedstrøms kamper — fjern lag som kom fra denne kampen
  // og nullstill resultater
  const nyeKamper = kamper.map(k => {
    if (!nullstillIds.has(k.id)) return k;

    // Behold lag som IKKE kom fra den redigerte kampen
    // (de kan ha kommet fra en annen kamp i samme runde)
  ...
  // Nullstill også den redigerte kampen selv
...
export async function avsluttTurnering(turneringId) {
  ...
  // Debug: logg nøkkeldata til konsoll
  console.log('[Rangering] lag:', t.lag?.map(l => ({ id: l.id, navn: l.navn })));
  console.log('[Rangering] A-kamper:', t.sluttspill?.A?.kamper?.map(k => ({
    id: k.id, ferdig: k.ferdig,
    lag1Id: k.lag1Id, lag2Id: k.lag2Id,
    lag1Poeng: k.lag1Poeng, lag2Poeng: k.lag2Poeng,
    vinnerId: k.vinnerId, taperId: k.taperId,
  })));

  const rangering = beregnEndeligRangering(t);
  console.log('[Rangering] beregnet:', rangering);

  const rangeringRen = rangering.map(r => ({
    plass: r.plass,
    lagId: r.lag?.id ?? r.lag?.lagId ?? null,
    navn:  r.lag?.navn ?? '?',
  })).filter(r => r.lagId);

  console.log('[Rangering] ren til Firestore:', rangeringRen);
