...
export async function registrerPuljeresultat(turneringId, puljeId, kampId, lag1Poeng, lag2Poeng, games = null) {
  ...
  // ── TILDEL VENTEBANE TIL KLARE KAMPER I NESTE RUNDE ──
  // Finn den ferdige kampens runde
  const ferdigKamp = nyePuljer.flatMap(p => p.kamper ?? []).find(k => k.id === kampId);
  const gjeldendeRunde = ferdigKamp?.runde ?? null;
  const antallBaner    = t.antallBaner ?? t.konfig?.antallBaner ?? 6;

  if (gjeldendeRunde != null) {
    const alleKamper = nyePuljer.flatMap(p => p.kamper ?? []);

    // Finn ledige baner: baner fra ferdige kamper i gjeldende runde
    const opptattIRunde = new Set();
    const ledigeBaneNr  = [];
    for (const k of alleKamper) {
      if (k.runde !== gjeldendeRunde) continue;
      if (k.ferdig && k.baneNr != null) ledigeBaneNr.push(k.baneNr);
      if (!k.ferdig) [k.lag1Id, k.lag2Id].filter(Boolean).forEach(id => opptattIRunde.add(id));
    }

    // Finn kamper i neste runde som er klare og ikke har venteBaneNr ennå
    const nesteRunde = gjeldendeRunde + 1;
    let baneIdx = 0;

    nyePuljer = nyePuljer.map(p => ({
      ...p,
      kamper: (p.kamper ?? []).map(k => {
        if (k.runde !== nesteRunde || k.ferdig || k.venteBaneNr != null) return k;
        if (opptattIRunde.has(k.lag1Id) || opptattIRunde.has(k.lag2Id)) return k;
        if (baneIdx >= ledigeBaneNr.length) return k;
        return { ...k, venteBaneNr: ledigeBaneNr[baneIdx++] };
      }),
    }));
  }
