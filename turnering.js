...
export function validerResultat(p1, p2, format) {
  ...
  // Normalt spill: taper er under points_to_win
  // Vinner må ha nøyaktig points_to_win (kampen stopper der)
  if (vinner !== points_to_win)
    return { ok: false, feil: `Ugyldig resultat: kampen avsluttes ved ${points_to_win} når motstanderen ikke har nådd ${points_to_win}.` };
  if (diff < win_by)
