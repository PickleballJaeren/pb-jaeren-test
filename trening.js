...
export async function bekreftNesteRunde() {
  ...
  try {
    ...
    if (erMix()) {
      ...
      // Bevar admin sin poeng-overstyring — hent fra gjeldende baneOversikt, ikke app.poengPerKamp
      const mp = gjeldBaneOversikt.find(b => b.baneNr === 1)?.maksPoeng
              ?? gjeldBaneOversikt.find(b => b.maksPoeng != null)?.maksPoeng
              ?? app.poengPerKamp ?? 15;
...
export async function avsluttTreningUI() {
  console.log('[avslutt] START — treningId:', app.treningId, 'db:', !!db);
  if (!db || !app.treningId) { visMelding('Økt ikke aktiv.', 'feil'); return; }
  ...
  // ── Hurtigsjekk lokalt: ingen poeng registrert ─────────────────────────────
  // Bruk kampstatus-cache (alltid oppdatert) for å unngå Firestore-kall når
  // ingen poeng er registrert. Sender signal til tilskuere umiddelbart.
  // Viktig: kun slå inn hvis cachen faktisk har kamper — en tom cache betyr
  // at kamp-lytteren ikke har lastet ennå, ikke at ingen poeng er registrert.
  const ksc = Object.values(_getKampStatusCache());
  const ingenPoengLokalt = ksc.length > 0 && ksc.every(k => !k?.ferdig);
  console.log('[avslutt] ksc.length:', ksc.length, 'ingenPoengLokalt:', ingenPoengLokalt,
    'ferdig-status:', ksc.map(k => k?.ferdig));
  if (ingenPoengLokalt) {
    try {
    ...
    sessionStorage.removeItem('aktivTreningId'); sessionStorage.removeItem('aktivTreningKlubbId');
    localStorage.removeItem('aktivTreningId');   localStorage.removeItem('aktivTreningKlubbId');
    ...
    app.venteliste   = [];
    _setKampStatusCache({});
    visMelding('Økt avsluttet uten registrerte poeng.', 'advarsel');
    frigiUI();
    stoppFailSafe();
    _naviger('slutt');
    return;
  ...
  console.log('[avslutt] Går inn i try-blokk');
  try {
    ...
    console.log('[avslutt] Låser treningsdokument…');
    await lassTrening(null);
    console.log('[avslutt] Lås OK');
    ...
    console.log('[avslutt] alleKamper:', alleKamper.length, 'medPoeng:', alleKamperMedPoeng.length,
      'scoringsFormat:', app.scoringsFormat,
      'eksempel:', alleKamper[0]);

    if (alleKamperMedPoeng.length === 0) {
      // Fallback: ingen kamper med poeng funnet i Firestore.
      // Mulig årsak: kamp-dokumenter ikke skrevet ennå, eller alle ble nullstilt.
      // Logg for debugging og avslutt med melding.
      console.warn('[avslutt] Ingen kamper med poeng — avslutter uten resultater.');
      await updateDoc(doc(db, SAM.TRENINGER, app.treningId), {
        status: 'avsluttet',
        avsluttetDato: serverTimestamp(),
        laast: false,
    ...
    const fullforteRunder = alleRunder
      .filter(rundeNr => {
        ...
        // En runde regnes som fullført om minst én kamp har poeng.
        // Manglende enkeltbaner (f.eks. singel i 6-spiller) ekskluderes fra
        // statistikken, men stopper ikke avslutningen.
    ...
    if (erMix()) {
      ...
      rangerteBaner = Object.keys(baneGrupper)
        ...
        .sort((a, b) => a - b)
        .map(baneNr => {
          // Bevar erSingel fra baneOversikt slik at visningen er korrekt
    ...
    console.log('[avslutt] sluttrangering:', sluttrangering.length,
      'fullforteRunder:', fullforteRunder,
      'rangerteBaner:', rangerteBaner?.length);

    if (sluttrangering.length === 0) {
      console.warn('[avslutt] Sluttrangering er tom — avslutter uten resultater.');
      const batchTomRang = writeBatch(db);
      batchTomRang.update(doc(db, SAM.TRENINGER, app.treningId), {
        status: 'avsluttet',
        avsluttetDato: serverTimestamp(),
        laast: false,
      });
      await batchTomRang.commit();
      sessionStorage.removeItem('aktivTreningId'); sessionStorage.removeItem('aktivTreningKlubbId');
      localStorage.removeItem('aktivTreningId');   localStorage.removeItem('aktivTreningKlubbId');
      try { history.replaceState(null, '', location.pathname); } catch (_) {}
      _stoppLyttere();
    ...
    // Commit rating/historikk/resultater først — UTEN status-endring.
    // Status settes i egen batch etterpå slik at lyttere (tilskuere, andre enheter)
    // ikke trigges før alle resultater er synlige i Firestore.
    batch.update(doc(db, SAM.TRENINGER, app.treningId), { laast: false });
    await batch.commit();

    // Nå er alle resultater skrevet — sett status til avsluttet.
    // Dette trigger onSnapshot hos alle lyttere, som da kan lese resultater trygt.
    await updateDoc(doc(db, SAM.TRENINGER, app.treningId), {
      status: 'avsluttet',
      avsluttetDato: serverTimestamp(),
    });
