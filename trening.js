...
export function visAvsluttModal() {
  _krevAdmin(
    ...
    () => {
      // Sjekk om pågående runde har uregistrerte kamper
      const cache = _getKampStatusCache();
      const uregistrerte = (app.baneOversikt ?? []).some(bane => {
        const k = cache[`bane${bane.baneNr}_1`];
        return !k?.ferdig;
      });

      const tekstEl = document.getElementById('modal-avslutt-tekst');
      if (tekstEl) {
        if (uregistrerte && !erMix()) {
          tekstEl.innerHTML = 'Ikke alle kamper i denne runden er registrert. '
            + '<strong style="color:var(--yellow)">Ufullstendige kamper telles ikke med i rangeringen.</strong> '
            + 'Tidligere fullførte runder brukes som grunnlag. Kan ikke angres.';
        } else {
...
export async function avsluttTreningUI() {
  ...
  try {
    ...
    if (alleKamperMedPoeng.length === 0) {
      visMelding('Ingen kamper med registrerte poeng funnet. Registrer minst én kamp før du avslutter.', 'advarsel');
      await lossTrening();
      return;
    }

    // ── Beregn sluttrangering på tvers av ALLE runder med registrerte poeng ──────────
    // En runde er "fullført" hvis ALLE kampene har poeng.
    // Hvis ingen runde er fullstendig (f.eks. avsluttet midt i runde), brukes
    // alle runder som har minst én kamp med poeng — slik at avslutning alltid er mulig.
    const alleRunder = [...new Set(alleKamper.map(k => k.rundeNr))];
    let fullforteRunder = alleRunder
      .filter(rundeNr => {
    ...
    // Fallback: ingen fullstendig runde — bruk alle runder med minst én registrert kamp
    if (fullforteRunder.length === 0) {
      fullforteRunder = alleRunder
        .filter(rundeNr => alleKamper.some(k => k.rundeNr === rundeNr && harPoeng(k)))
        .sort((a, b) => a - b);
    }
