...
export async function avsluttTreningUI() {
  ...
  try {
    ...
    if (alleKamperMedPoeng.length === 0) {
      // Ingen poeng registrert — avslutt likevel slik at alle enheter (tilskuere)
      // navigeres bort via onSnapshot-lytteren. Ingen rating eller historikk skrives.
      const batchTom = writeBatch(db);
      batchTom.update(doc(db, SAM.TRENINGER, app.treningId), {
        status: 'avsluttet',
        avsluttetDato: serverTimestamp(),
        laast: false,
      });
      await batchTom.commit();
