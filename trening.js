...
export async function bekreftNesteRunde() {
  ...
  try {
    ...
    batch.update(doc(db, SAM.TRENINGER, app.treningId), {
      ...
      venteliste: nyVenteliste,
      laast: false,
      visResultat: false,   // nullstilles atomisk med ny runde — unngår re-trigger av resultatskjerm
    });
    baneOversikt.forEach(bane =>
    ...
    // Marker lokalt at visResultat skal ignoreres — forhindrer at admin
    // trigges til resultatskjermen av sin egen lytter etter rundebytte
    app._ignorerVisResultat = true;
