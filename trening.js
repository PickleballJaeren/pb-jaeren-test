...
export async function avsluttTreningUI() {
  ...
  try {
    ...
    if (alleKamperMedPoeng.length === 0) {
      // Fallback: cache sa ingen poeng, men Firestore bekrefter det samme.
      // Samme avslutningslogikk som hurtigsjekken over.
      await updateDoc(doc(db, SAM.TRENINGER, app.treningId), {
        status: 'avsluttet',
        avsluttetDato: serverTimestamp(),
        laast: false,
    ...
    if (sluttrangering.length === 0) {
      // Sluttrangering tom — avslutt likevel slik at alle enheter navigeres bort.
