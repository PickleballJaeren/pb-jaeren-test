...
// ════════════════════════════════════════════════════════
// LYTTERE — delegerer til lyttere.jsfunction visNesteRundeModal() {
  _krevAdmin(
...
window.visNesteRundeModal = visNesteRundeModal;async function bekreftNesteRunde() {
...
window.bekreftNesteRunde = bekreftNesteRunde;function visAvsluttModal() {
  _krevAdmin(
    'Avslutt økt',
    erMix()
...
window.visAvsluttModal = visAvsluttModal;async function avsluttTreningUI() {
  if (!db || !app.treningId) { visMelding('Økt ikke aktiv.', 'feil'); return; }
...
window.avsluttTreningUI = avsluttTreningUI;// NY ØKT
// ════════════════════════════════════════════════════════
function nyTrening() {
  _stoppLyttere();
  app.valgtIds.clear();
...
async function utforAvbrytOkt() {
  ...
  try {
    ...
    _getSisteDeltakereApen() = false;
    setTimeout(() => {
      if (!_getSisteDeltakereApen()) _toggleSisteDeltakere();
    }, 300);
