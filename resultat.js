...
import {
  ...
  collection, doc, updateDoc, setDoc, serverTimestamp,
  query, where, getDocs,
} from './firebase.js';
...
export async function visRundeResultat() {
  ...
  // Synkroniser alle brukere til resultat-skjermen via skjermSync.
  // Eget dokument — unngår konflikter med låsemekanismen i treningsdokumentet.
  if (db && app.treningId) {
    try {
      await setDoc(doc(db, SAM.SKJERMSYNC, app.treningId), {
        status: 'resultat',
        ts:     serverTimestamp(),
      });
    } catch (_) {}
