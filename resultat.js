...
// ════════════════════════════════════════════════════════
// HTML-BYGGERE — private hjelpefunksjoner for resultat-skjermene
// ════════════════════════════════════════════════════════

/**
 * Bygger én rang-rad for Mix-resultatlisten.
 * @param {object} s   — { navn, seire, for, imot }
 * @param {number} i   — 0-basert indeks (brukes til rang-klasse)
 */
function _byggMixRangRad(s, i) {
...
/**
 * Bygger én rang-rad for konkurranse-resultatlisten (per bane).
 * @param {object} s          — spillerstatistikk
 * @param {object} forflytning — { retning, antall } eller null
 * @param {number} i          — 0-basert indeks
 */
function _byggKonkurranceRangRad(s, forflytning, i) {
  const rkl = ['rn-1', 'rn-2', 'rn-3', 'rn-4'][i] ?? '';
  let fm = '';
  if (forflytning) {
    const fmKl  = forflytning.retning === 'opp' ? 'fm-opp' : forflytning.retning === 'ned' ? 'fm-ned' : 'fm-blir';
    const fmTxt = forflytning.retning === 'opp' ? `↑ ${forflytning.antall}` : forflytning.retning === 'ned' ? `↓ ${forflytning.antall}` : '–';
    fm = `<div class="forflytning-merke ${fmKl}">${fmTxt}</div>`;
  }
  return `<div class="rang-rad">
    <div class="rang-nummer ${rkl}">${i + 1}</div>
    <div class="rang-navn">${escHtml(s.navn)}</div>
...
export async function visRundeResultat() {
  ...
  if (erMix()) {
    ...
    const rader = alleSpillere.map((s, i) => _byggMixRangRad(s, i)).join('');
    ...
    document.getElementById('resultat-innhold').innerHTML = (app.rangerteBAner ?? []).map(bane => {
      ...
      const bane5merke     = er5bane ? `<span class="bane-merke bane-merke-5spl">5 SPL</span>` : '';
      const singelMerkeRes = bane.erSingel ? `<span class="bane-merke bane-merke-singel">🏃 SINGEL</span>` : (app.er6SpillerFormat ? `<span class="bane-merke bane-merke-dobbel">🎾 DOBBEL</span>` : '');
      ...
      return `<div class="kort">
        <div class="kort-hode">
          <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap">
            <span class="res-bane-nummer" style="color:${baneNummerFarge}">${bane.baneNr}</span>
            <span class="res-bane-etikett">Bane ${baneIkon}</span>
...
export async function visSluttresultat() {
  ...
  if (!data.length && db) {
    document.getElementById('ledertavle').innerHTML =
      '<div class="tom-tilstand">Laster resultater…</div>';
    try {
  ...
  if (!data.length) {
    document.getElementById('ledertavle').innerHTML =
      '<div class="tom-tilstand">Ingen økt avsluttet ennå</div>';
...
function visKonkurranseSluttresultat(data) {
  ...
  document.getElementById('ledertavle').innerHTML = data.map(s => {
    ...
    return `<div class="lb-rad" onclick="apneProfil('${s.spillerId}')">
      ...
      <div style="text-align:right">
        <div class="res-rating-mono">${s.nyRating}</div>
  ...
  if (ratingEl) ratingEl.innerHTML = data.map(s => `
    <div class="lb-rad">
      <div class="lb-navn" style="font-size:17px">${s.navn ?? 'Ukjent'}</div>
      <div class="res-rating-mono" style="margin-right:10px">${s.ratingVedStart ?? STARTRATING} → ${s.nyRating}</div>
      <div class="lb-endring ${s.endring >= 0 ? 'pos' : 'neg'}">${s.endring >= 0 ? '+' : ''}${s.endring}</div>
    </div>`).join('');
}
