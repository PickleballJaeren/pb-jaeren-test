...
export async function visRundeResultat() {
  ...
  // Synkroniser alle brukere til resultat-skjermen.
  // Bruker serverTimestamp slik at Firestore alltid ser en endring,
  // selv om admin navigerer hit flere ganger.
  if (db && app.treningId) {
    try {
      await updateDoc(doc(db, SAM.TRENINGER, app.treningId), {
        skjermStatus:    'resultat',
        skjermStatusTs:  serverTimestamp(),
      });
    } catch (_) {}
  }
