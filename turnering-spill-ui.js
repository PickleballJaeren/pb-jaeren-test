...
function bracketKampHTML(kamp, lagMap, nivaa) {
  ...
  return `
    <div class="kamp-rad" style="margin-bottom:8px;cursor:${kanReg ? 'pointer' : 'default'}"
      ...
      ${kamp.ferdig
        ? `<div class="poeng-kolonne" style="font-size:18px;cursor:pointer;opacity:0.85"
             onclick="event.stopPropagation();redigerSluttspillKamp('${nivaa}','${escHtml(kamp.id)}')"
             title="Trykk for å redigere">
             <span>${kamp.lag1Poeng}</span><span>${kamp.lag2Poeng}</span>
           </div>`
