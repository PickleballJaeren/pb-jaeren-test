...
function _oppdaterDeltakerModalInnhold() {
  ...
  // Alle spillere som ikke er med i økt i det hele tatt
      ...
      (bane.spillere ?? []).forEach(s => {
        ...
        html += _deltakerRad(s, skalFjernes
          ...
          skalFjernes
            ...
            : `<button class="knapp knapp-liten" style="background:rgba(220,38,38,.15);color:var(--red2);border:1px solid rgba(220,38,38,.3);font-size:13px;padding:5px 10px" onclick="_fjernFraOkt('${s.id}')">Ta ut</button>`
        );
      });
    });
  }
  ...
  if (utenfor.length > 0) {
    ...
    utenfor.forEach(s => {
      html += _deltakerRad(s,
        ...
        `<button class="knapp knapp-gronn knapp-liten" style="font-size:13px;padding:5px 10px" onclick="_leggTilIOkt('${s.id}')">+ Legg til</button>`
      );
    });
  }

  if (!html) {
    html = `<div style="padding:20px;text-align:center;color:var(--muted2)">Ingen spillere å vise.</div>`;
  }
