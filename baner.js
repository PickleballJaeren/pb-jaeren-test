...
export function visBaner() {
  ...
  document.getElementById('bane-liste').innerHTML = (app.baneOversikt ?? []).map(bane => {
    ...
    if (erMix()) {
      ...
      return `<div class="kort">
        ...
        <div class="kort-innhold">
          <div class="kamp-rad" style="cursor:pointer" onclick="apneKampInput(${bane.baneNr}, 1)">
            <div style="flex:1">
    ...
    const rader = parter.map(par => {
      ...
      return `<div class="kamp-rad" style="cursor:pointer" onclick="apneKampInput(${bane.baneNr}, ${par.nr})">
...
// ════════════════════════════════════════════════════════
// KAMP-FOR-KAMP — åpner poengregistrering for én enkelt kamp
// ════════════════════════════════════════════════════════

/**
 * Åpner poengregistrering for én spesifikk kamp på en bane.
 * Etter autosave navigeres tilbake til baneoversikten automatisk.
 * @param {number} baneNr  — banenummer
 * @param {number} kampNr  — kampnummer (par.nr, starter på 1)
 */
export function apneKampInput(baneNr, kampNr) {
  const bane = (app.baneOversikt ?? []).find(b => b.baneNr === baneNr);
  ...
  // Allerede ferdig — vis likevel slik at poeng kan redigeres
  app.aktivBane    = baneNr;
  app.aktivKampNr  = kampNr; // lagres for autolagre-callback
  ...
  const s   = bane.spillere;
  const k   = kampStatusCache[`bane${baneNr}_${kampNr}`];
  const e   = k?.ferdig ? { l1: k.lag1Poeng, l2: k.lag2Poeng } : null;
  const i   = 0; // alltid kampIdx 0 siden vi viser én kamp
  ...
  if (erSingelBane) {
    const l1n = s[0]?.navn ?? '?';
    const l2n = s[1]?.navn ?? '?';
    kampHTML = `<div class="kamp-kort" id="kk-0" data-maks="${maksPoeng}">
      <div class="kamp-hode">
        🏃 Singel <span class="kamp-merke" style="background:rgba(234,179,8,.15);color:var(--yellow)">1 vs 1</span>
        <div class="kamp-status${e ? ' lagret' : ''}" id="kamp-status-0">${e ? '✓ Lagret' : ''}</div>
      </div>
      ...
      <div class="lag-rad">
        <div class="lag-boks">
          <div class="lag-navn" style="color:var(--white);font-weight:600">${escHtml(l1n)}</div>
          <input type="hidden" id="s0_l1" value="${e ? e.l1 : ''}"/>
          <div class="poeng-velger-boks" id="pvb_0_l1" onclick="_apnePicker(0,'l1')">${e != null ? e.l1 : '–'}</div>
          <div class="poeng-picker" id="pp_0_l1" style="display:none"></div>
        </div>
        <div class="vs-deler">–</div>
        <div class="lag-boks">
          <div class="lag-navn" style="color:var(--white);font-weight:600">${escHtml(l2n)}</div>
          <input type="hidden" id="s0_l2" value="${e ? e.l2 : ''}"/>
          <div class="poeng-velger-boks" id="pvb_0_l2" onclick="_apnePicker(0,'l2')">${e != null ? e.l2 : '–'}</div>
          <div class="poeng-picker" id="pp_0_l2" style="display:none"></div>
        </div>
      </div>
    </div>`;
  } else {
    ...
    kampHTML = `<div class="kamp-kort" id="kk-0" data-maks="${maksPoeng}">
      <div class="kamp-hode">
        Kamp ${kampNr} <span class="kamp-merke">Americano</span>
        <div class="kamp-status${e ? ' lagret' : ''}" id="kamp-status-0">${e ? '✓ Lagret' : ''}</div>
      </div>
      ${hvilerHTML}
      <div class="lag-rad">
  ...
  document.getElementById('poeng-kamper').innerHTML = kampHTML;
  _naviger('poeng');
  oppdaterPoengNav();

  // Åpne picker for venstre spiller automatisk om kampen ikke er ferdig
  if (!e) {
    setTimeout(() => _apnePicker(0, 'l1'), 180);
  }
}
window.apneKampInput = apneKampInput;
