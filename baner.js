...
function _apneBestAv3Modal(bane, parOverstyr) {
  ...
  // Sjekk om kamp allerede er lagret
  const cached = kampStatusCache[`bane${bane.baneNr}_${par.nr}`];
  if (cached?.ferdig && cached?.games) {
    _b3Games = [...(cached.games ?? []), null, null, null].slice(0, 3);
  }
...
window.b3VelgPoeng = function(felt, verdi) {
  ...
  if (game.l1 != null && game.l2 != null) {
    ...
    // - Vinner må ha minst 11 poeng
    // - Vinner må ha minst 2 mer enn taper
    // - Tak på 15 (ved 14-14 vinner første til 15)
    const høy  = Math.max(game.l1, game.l2);
    const lav  = Math.min(game.l1, game.l2);
    const diff = høy - lav;
    // Gyldige resultater per pickleball-regler:
    // - Normal seier: vinner har 11, taper maks 9 (aldri deuce)
    // - Deuce-seier: begge nådde 10, vinn med nøyaktig 2 (12-10, 13-11, 14-12, 15-13)
    // - Tak: 15-14 (eneste gyldige 1-differanse)
      ...
      const feilTekst = høy < 11
        ...
        : høy > 15
          ...
          : lav === 10 && høy === 11
            ...
            : diff !== 2 && !(høy === 15 && diff === 1)
              ? `Ugyldig differanse — gyldige resultater: 11-0 til 11-9, 12-10, 13-11, 14-12, 15-13, 15-14.`
              : 'Ugyldig resultat — sjekk reglene.';
...
function _b3OppdaterUI() {
  ...
  if (indEl) {
    indEl.innerHTML = [0,1,2].map(i => {
      const g = _b3Games[i];
      let stil = 'width:14px;height:14px;border-radius:50%;border:2px solid var(--border2);display:inline-block;transition:background .2s';
      if (g)        stil += g.l1 > g.l2 ? ';background:var(--green);border-color:var(--green)' : ';background:var(--red2);border-color:var(--red2)';
      else if (i === _b3AktivGame) stil += ';border-color:var(--accent);background:rgba(37,99,235,.2)';
      return `<span style="${stil}"></span>`;
    }).join('');
  }
...
window.b3LagreKamp = async function() {
  ...
  const lag1Poeng = stilling.lag1; // antall games vunnet
  ...
  try {
    ...
    batch.update(doc(db, SAM.KAMPER, kampId), {
      lag1Poeng, lag2Poeng, ferdig: true, games,
    });
    await batch.commit();

    // Oppdater cache
    kampStatusCache[`bane${bane.baneNr}_${par.nr}`] = {
      id: kampId, ferdig: true, lag1Poeng, lag2Poeng, games,
