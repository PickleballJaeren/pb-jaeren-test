...
window._lagreRedigerBaner = async function() {
  ...
  try {
    ...
    // Oppdater maksPoeng basert på faktisk antall spillere etter redigering
    // 5-spillerbane: redusert poengsum (3/5 av normal), 4-spillerbane: full poengsum
    const mp = app.poengPerKamp ?? 17;
    _redigerBaner.forEach(b => {
      const n = b.spillere?.length ?? 0;
      if (n === 5) b.maksPoeng = Math.round(mp * 3 / 5);
      else if (n === 4) b.maksPoeng = mp;
      // singel (2) og andre formater beholdes uendret
    });
  ...
  el.innerHTML = treff.map(s =>
    `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border)">
      ...
      <button class="knapp knapp-gronn knapp-liten" style="font-size:13px;padding:5px 10px"
        onclick="_leggTilIOkt('${s.id}')">+ Legg til</button>
    </div>`
  ).join('');
}
