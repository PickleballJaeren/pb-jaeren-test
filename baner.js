...
export function visBaner() {
  ...
  document.getElementById('bane-liste').innerHTML = (app.baneOversikt ?? []).map(bane => {
    ...
    if (erMix()) {
      ...
      return `<div class="kort" class="bane-kort-klikk" onclick="apnePoenginput(${bane.baneNr})">
        <div class="kort-hode">
          <div class="bane-hode-rad">
            <div class="bane-nummer-stor bane-num-mix">${bane.baneNr}</div>
            <div>
              <div class="bane-etikett">Bane ${spillTilMerke}</div>
    ...
    if (erSingelBane) {
      ...
      return `<div class="kort" class="bane-kort-klikk" onclick="apnePoenginput(${bane.baneNr})">
        <div class="kort-hode">
          <div class="bane-hode-rad">
            <div class="bane-nummer-stor bane-num-singel">${bane.baneNr}</div>
            <div>
              <div class="bane-etikett">Singel ${singelMerke} ${spillTilMerke}</div>
              <div style="font-size:15px;color:${ferdig?'var(--green2)':'var(--muted2)'};font-weight:600">${ferdig?'✓ Ferdig':'Mangler poeng'}</div>
            </div>
          </div>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
    ...
    const bane5merke = antallSpillere === 5
      ? `<span class="bane-merke bane-merke-5spl">5 SPL</span>`
      : '';
    const dobbelMerke = app.er6SpillerFormat
      ? `<span class="bane-merke bane-merke-dobbel">🎾 DOBBEL</span>`
      : '';
    ...
    return `<div class="kort" class="bane-kort-klikk" onclick="apnePoenginput(${bane.baneNr})">
      <div class="kort-hode">
        <div class="bane-hode-rad">
          ...
          <div>
            <div class="bane-etikett">Bane ${bane5merke} ${dobbelMerke} ${spillTilMerke}</div>
  ...
  document.getElementById('poeng-kamper').innerHTML = parter.map((par, i) => {
    ...
    if (erSingelBane) {
      ...
      return `<div class="kamp-kort" id="kk-${i}" data-maks="${bane.maksPoeng ?? app.poengPerKamp ?? 15}">
        ...
        <div class="lag-rad">
          <div class="lag-boks">
            <div class="lag-navn lag-navn-hvit">${escHtml(l1n)}</div>
          ...
          <div class="lag-boks">
            <div class="lag-navn lag-navn-hvit">${escHtml(l2n)}</div>
