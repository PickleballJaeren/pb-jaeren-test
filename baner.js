...
export function visBaner() {
  ...
  document.getElementById('bane-liste').innerHTML = (app.baneOversikt ?? []).map(bane => {
    ...
    // ── Mix & Match ──────────────────────────────────────────────────────────
    // Én kamp per bane. To varianter:
    //   • Singel-bane (6-spiller-format): 1 vs 1 — erSingelBane === true
    //   • Dobbel-bane (alle andre Mix):   2 vs 2
    if (erMix()) {
...
export function apnePoenginput(baneNr) {
  ...
  // Mix: singel-bane bruker PARTER_6_SINGEL (1 vs 1), dobbel-bane bruker én fast 2v2-kamp
  const erDobbelBane6 = app.er6SpillerFormat && (bane?.erDobbel === true);
  const parter = erMix()
    ? (erSingelBane
        ? PARTER_6_SINGEL                          // Mix singel: 1 vs 1
        : [{ nr: 1, lag1: [0, 1], lag2: [2, 3] }] // Mix dobbel: 2 vs 2
      )
