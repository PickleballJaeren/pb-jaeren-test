  ...
  try {
    ...
    container.innerHTML = spillere.map(navn => {
      ...
      return `<div class="ks-spiller-rad${valgt ? ' valgt' : ''}" data-navn="${escHtml(navn)}">
        <span class="ks-spiller-rad-navn">${escHtml(navn)}</span>
        <div class="ks-spiller-rad-knapper">
          <button class="ks-ab-knapp${erA ? ' aktiv-a' : ''}" onclick="velgSpillerA('${escHtml(navn)}')" title="Velg som Spiller A">A</button>
          <button class="ks-ab-knapp" onclick="velgSpillerB('${escHtml(navn)}')" title="Velg som Spiller B (lager lag med A)">B</button>
        </div>
      </div>`;
    }).join('');
  } catch (e) {
...
function oppdaterABStatus() {
  // Oppdater A-knapper og statuslinje
  document.querySelectorAll('.ks-ab-knapp').forEach(btn => {
