...
export async function apneProfil(spillerId) {
  ...
  // Nullstill til Americano-fane og lagre spillerId
...
window.byttProfilFane = function(fane) {
  ...
  if (fane === 'singel' && _profilSpillerId) _lastProfilSingelHistorikk(_profilSpillerId, false);
};
...
window.byttGlobalProfilFane = function(fane) {
  ...
  if (fane === 'singel' && _globalProfilSpillerId) _lastProfilSingelHistorikk(_globalProfilSpillerId, true);
};

async function _lastProfilSingelHistorikk(spillerId, erGlobal) {
  ...
  try {
    // Hent singelhistorikk fra Firestore
    ...
    // Hent gjeldende singlerating fra spillerdokument
    ...
    // Vis rating
    if (ratingEl) {
      ratingEl.textContent = singelRating != null ? singelRating : '—';
    }
    ...
    if (diagramEl && historikk.length) {
      ...
      if (erGlobal) {
        _globalSingelDiagram = lagRatingDiagram(diagramEl, ratingData, etiketter, _globalSingelDiagram);
      } else {
        _profilSingelDiagram = lagRatingDiagram(diagramEl, ratingData, etiketter, _profilSingelDiagram);
      }
    } else if (diagramEl) {
  ...
  try {
    ...
    if (!sessionStorage.getItem('utf-avvist-toast-vist')) {
      ...
      if (nyligAvvist.length > 0) {
        ...
        setTimeout(() => visMelding(`${motNavn} avslo utfordringen din. 🐥`, 'advarsel'), 1500);
      }
    }
  } catch (e) {
...
// ════════════════════════════════════════════════════════
// SANNTIDSLYTTER — utfordringer
// Startes når utfordrer-skjermen åpnes, stoppes ved navigering bort.
// ════════════════════════════════════════════════════════
...
export function startUtfordrerLytter() {
  ...
  _utfordringLytterAvmeld = onSnapshot(
    ...
    async (snap) => {
      console.log('[UtfordrerLytter] endring mottatt, antall dok:', snap.size);
      // Oppdater begge lister ved enhver endring i utfordringer-samlingen
    ...
    feil => console.error('[UtfordrerLytter] FEIL:', feil?.message ?? feil, feil?.code)
  );
}
...
window.toggleSingelRanking = function() {
  ...
  // Last data første gang panelet åpnes
...
let _oppdatererVelger = false; // blokkerer onchange under programmatisk oppdatering

export async function visUtfordrerSkjerm() {
  const klubbId  = _getAktivKlubbId();

  // Hent ferske spillerdata fra Firestore for å sikre at singelRating/singelKamper
  // er oppdatert etter registrerte singelkamper — app.spillere kan være utdatert
  ...
  if (db && klubbId) {
    try {
      ...
      if (!snap.empty) {
        spillere = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      }
    } catch (_) { /* fall back to app.spillere */ }
  }
...
async function _lastAktiveUtfordringer(klubbId, spillere) {
  ...
  if (!db || !klubbId) return;
  // el kan være null når modal åpnes fra global-profil — fortsett likevel for å oppdatere listen om den finnes
  ...
  try {
    ...
    if (!el) return; // skjermen ikke aktiv — refresh skjedde stille
    if (!alle.length) {
    ...
    el.innerHTML = alle.map(u => {
      ...
      // Game-poeng per game fra involvert spillers perspektiv
      ...
      const gamesHTML = gamesSpilt.length
        ? `<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:6px">` +
          gamesSpilt.map((g, i) => {
            ...
            return `<span style="font-size:12px;background:rgba(255,255,255,.06);border-radius:6px;padding:2px 8px;font-family:'DM Mono',monospace;color:${farge}">` +
                   `G${i+1}: ${megPoeng}–${demPoeng}</span>`;
          }).join('') +
      ...
      if (erPagar && gamesSpilt.length) {
        ...
        // Andre spilleres kamper — vis fra utfordrerens perspektiv
      ...
      if (!erPagar && erMotstander) {
        ...
        // Pågår og du er involvert og serien ikke avgjort — vis Registrer game
...
function _visSingelRanking(spillere) {
  ...
  el.innerHTML = medSingel.map((s, i) => {
    ...
    // Kilde-indikator: 🎾 = ekte singelrating, 🏸 = midlertidig Americano-rating
...
function _utfVelgPoeng(felt, verdi) {
  ...
  // Åpne det andre feltets grid automatisk
  const annet = felt === 'p1' ? 'p2' : 'p1';
  const annetPicker = document.getElementById(`utf-pp-${annet}`);
  const annetBoks   = document.getElementById(`utf-pvb-${annet}`);
  // Åpne kun hvis det andre feltet ikke allerede har en verdi
  if (annetPicker && annetBoks && annetPicker.dataset.valgt == null) {
  ...
  try {
    ...
    if (erFerdig) {
      ...
      if (utfSpillerSnap.exists()) {
        ...
        // Skriv singelhistorikk-rad for utfordreren
      ...
      if (motSpillerSnap.exists()) {
        ...
        // Skriv singelhistorikk-rad for motstanderen
        batch.set(doc(collection(db, SAM.SINGEL_HISTORIKK)), {
    ...
    if (erFerdig) {
      ...
      setTimeout(() => window.utfApnePicker('p1'), 80);
      // Refresh aktivlisten umiddelbart — viser oppdatert game-stilling
      const klubbId = _getAktivKlubbId();
