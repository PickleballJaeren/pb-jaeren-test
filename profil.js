...
export async function utforNullstill() {
  ...
  try {
    ...
    for (const d of spillerSnap.docs) await bh.oppdater(d.ref, { rating: STARTRATING });
    await bh.kommit();

    // 2. Slett all ratinghistorikk
    const histSnap = await getDocs(collection(db, SAM.HISTORIKK));
    const bhHist = lagBatchHjelper(db);
    for (const d of histSnap.docs) await bhHist.slett(d.ref);
    await bhHist.kommit();

    // 3. Slett alle resultater
    const resSnap = await getDocs(collection(db, SAM.RESULTATER));
    const bhRes = lagBatchHjelper(db);
    for (const d of resSnap.docs) await bhRes.slett(d.ref);
    await bhRes.kommit();
