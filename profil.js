...
// ════════════════════════════════════════════════════════
const UTF_RATING_VINDU    = 150;   // maks ratingdiff for å utfordre
...
export async function visUtfordrerSkjerm() {
  ...
  // ── Aktive utfordringer ────────────────────────────────
  await _lastAktiveUtfordringer(klubbId, spillere);

  // ── Singel-ranking ─────────────────────────────────────
  _visSingelRanking(spillere);

  // ── Siste resultater ───────────────────────────────────
  await _lastSisteResultater(klubbId, spillere);
}
...
window.sendUtfordringFraSkjerm = async function() {
  ...
  // Bruk eksisterende sendUtfordring-logikk
  sessionStorage.setItem('aktivSpillerId', megId);
  await window.sendUtfordring(motId);
...
async function _lastAktiveUtfordringer(klubbId, spillere) {
  ...
  try {
    ...
    el.innerHTML = alle.map(u => {
      ...
      const stillingHTML = erPagar && u.games?.length
        ? `<span style="color:var(--green2);font-weight:600">${utfSeire}</span> – <span style="color:var(--red2);font-weight:600">${motSeire}</span> · Game ${u.games.length + 1} gjenstår`
        : erPagar ? 'Starter snart' : `Sendt ${Math.floor(_dagerSiden(u.opprettet))} dager siden · utløper om ${UTF_UTLOP_DAGER - Math.floor(_dagerSiden(u.opprettet))} dager`;
      return `<div class="${erPagar ? 'utf-aktiv-kort' : 'utf-venter-kort'}">
...
function _visSingelRanking(spillere) {
  ...
  const medSingel = spillere
    .filter(s => s.singelRating != null)
    .sort((a, b) => (b.singelRating ?? STARTRATING) - (a.singelRating ?? STARTRATING));

  if (!medSingel.length) {
    el.innerHTML = '<div class="tom-tilstand-liten" style="text-align:center">Ingen singel-kamper spilt ennå</div>';
    return;
  }

  el.innerHTML = medSingel.map((s, i) => {
    const ini    = lagInitialer(s.navn);
    const rating = s.singelRating ?? STARTRATING;
    ...
    return `<div class="utf-singel-rad">
      ...
      <div class="lb-navn" style="flex:1;cursor:pointer" onclick="apneGlobalProfil('${s.id}')">${escHtml(s.navn ?? 'Ukjent')}</div>
...
async function _lastSisteResultater(klubbId, spillere) {
  ...
  try {
    ...
    const ferdig = snap.docs.map(d => ({ id: d.id, ...d.data(), _type: 'ferdig' }));
    ...
    el.innerHTML = alle.map(u => {
      ...
      if (u._type === 'avvist') {
        return `<div class="utf-res-rad">
          <div style="flex:1">
            <div style="font-size:14px;color:var(--white)">${escHtml(u.motstanderNavn)} avslo ${escHtml(u.utfordrerNavn)} 🐥</div>
...
async function _kanUtfordre(utfordrerSpiller, motstanderSpiller, klubbId) {
  ...
  const utfRating = utfordrerSpiller.rating ?? STARTRATING;
  const motRating = motstanderSpiller.rating  ?? STARTRATING;
  const diff      = Math.abs(utfRating - motRating);

  // Sjekk ratingsone — unntak: spilleren rett over deg
  const alleSpillere = [...(window._app?.spillere ?? [])].sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));
...
async function _sendUtfordring(utfordrerSpiller, motstanderSpiller, klubbId) {
  await addDoc(collection(db, SAM.UTFORDRINGER), {
    ...
    utfordrerRating:   utfordrerSpiller.rating ?? STARTRATING,
    motstanderId:      motstanderSpiller.id,
    motstanderNavn:    motstanderSpiller.navn   ?? 'Ukjent',
    motstanderRating:  motstanderSpiller.rating ?? STARTRATING,
...
window.registrerUtfordringGame = function(utfordringId, erUtfordrer) {
  // Åpne game-modal
  const modal = document.getElementById('modal-utf-game');
  if (!modal) return;
  modal.dataset.utfordringId = utfordringId;
  modal.dataset.erUtfordrer  = erUtfordrer;
  document.getElementById('utf-game-p1').value = '';
  document.getElementById('utf-game-p2').value = '';
  document.getElementById('utf-game-feil').textContent = '';
  modal.style.display = 'flex';
  document.getElementById('utf-game-p1').focus();
};
...
window.bekreftUtfordringGame = async function() {
  ...
  const p1 = parseInt(document.getElementById('utf-game-p1').value, 10);
  const p2 = parseInt(document.getElementById('utf-game-p2').value, 10);
  ...
  try {
    ...
    if (erFerdig) {
      ...
      if (utfSpillerSnap.exists()) {
        const gammelRating = utfSpillerSnap.data().singelRating ?? STARTRATING;
        batch.update(doc(db, SAM.SPILLERE, u.utfordrerIds), {
          singelRating: Math.max(1, gammelRating + utfDelta),
        });
      }
      if (motSpillerSnap.exists()) {
        const gammelRating = motSpillerSnap.data().singelRating ?? STARTRATING;
        batch.update(doc(db, SAM.SPILLERE, u.motstanderId), {
          singelRating: Math.max(1, gammelRating + motDelta),
        });
      }
      await batch.commit();
