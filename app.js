...
function _lyttereCallbacks() {
  return {
    ...
    onNyRunde:           ()  => { setKampStatusCache({}); naviger('baner'); },
    onOktAvsluttet:      ()  => naviger('slutt'),
    onVisResultater:     ()  => naviger('slutt'),
...
function startLyttere() {
  // Start skjermSync-lytter — eget dokument, unngår låsekonflikter
  startSkjermSyncLytter(app.treningId, {
    onVisRundeResultat: async () => { await visRundeResultat(); },
    onVisResultater:    ()  => naviger('slutt'),
    onNyRunde:          ()  => { setKampStatusCache({}); naviger('baner'); },
  });
