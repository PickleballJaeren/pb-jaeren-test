...
function byttKlubb(klubbId) {
  ...
  if (!klubbId || !KLUBBER[klubbId]) {
    aktivKlubbId = null;
    sessionStorage.removeItem('aktivKlubbId');
    oppdaterKlubbUI();
    return;
  }
  aktivKlubbId = klubbId;
  sessionStorage.setItem('aktivKlubbId', klubbId);
...
document.addEventListener('visibilitychange', async () => {
  ...
  // Gjenopprett klubbvalg i minnet hvis det ble nullstilt i bakgrunnen.
  // Kaller IKKE initEtterKlubbValg() — det forstyrrer en allerede aktiv tilstand.
  // Kun variabelen og UI-en gjenopprettes, slik at PIN-koden stemmer.
  if (!aktivKlubbId) {
    const lagretKlubbId = sessionStorage.getItem('aktivKlubbId');
    if (lagretKlubbId && KLUBBER[lagretKlubbId]) {
      aktivKlubbId = lagretKlubbId;
      setErAdmin(KLUBBER[lagretKlubbId].demo);
      _setSpillereKlubbId(lagretKlubbId);
      oppdaterKlubbUI();
    }
  }
