...
window._app = app; // brukes av utfordrermodusen for ratingsone-sjekk
...
window.settAktivSpiller = function(spillerId) {
  if (spillerId) {
    sessionStorage.setItem('aktivSpillerId', spillerId);
  } else {
    sessionStorage.removeItem('aktivSpillerId');
  }
};

/** Oppdaterer spillervelgeren i utfordrermodusen med aktuelle spillere. */
function oppdaterAktivSpillerVelger() {
  const velger = document.getElementById('aktiv-spiller-velger');
  if (!velger) return;
  ...
  velger.innerHTML = '<option value="">— Velg deg selv —</option>' +
    (app.spillere ?? [])
      ...
      .map(s => `<option value="${s.id}" ${s.id === lagretId ? 'selected' : ''}>${s.navn ?? 'Ukjent'}</option>`)
      .join('');
}
...
async function init() {
  ...
  // Vis hjemskjerm alltid ved oppstart
  naviger('hjem');

  // Les ?klubb= fra URL — tillater direktelenker til økt eller turnering.
  // byttKlubb() er synkron og starter initEtterKlubbValg() som håndterer
  // ?okt= og ?turnering= videre.
  const _oppstartParams = new URLSearchParams(location.search);
  const _urlKlubbId     = _oppstartParams.get('klubb');
  if (_urlKlubbId && KLUBBER[_urlKlubbId]) {
    byttKlubb(_urlKlubbId);
  }
}
...
async function initEtterKlubbValg() {
  ...
  try {
    // Steg 0: sjekk URL-parametere ?okt= og ?turnering= (delt lenke)
    ...
    if (urlTurneringId) {
      try {
        // Naviger direkte til turneringen — URL er allerede satt
        await apneTurneringFraLenke(urlTurneringId);
        visMelding('Turnering åpnet!');
        return;
      } catch (e) {
        console.warn('[Init] Kunne ikke åpne turnering fra lenke:', e?.message);
        try { history.replaceState(null, '', location.pathname); } catch (_) {}
      }
    }
