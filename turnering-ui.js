...
window.avsluttTurneringUI = function() {
  _krevAdmin('Avslutt turnering', 'Turneringen avsluttes og endelig rangering beregnes.', async () => {
    try {
      ...
      // Vent litt på Firestore så hentTurnering får oppdatert data
      await new Promise(r => setTimeout(r, 800));
      const t = await hentTurnering(_aktivTurneringId);
      app.aktivTurnering = t;
      // Inject rangering direkte om Firestore ikke har oppdatert ennå
      if (!t.rangering?.length && rangeringRen?.length) {
        t.rangering = rangeringRen;
      }
...
export async function visResultat(turnering) {
  // Bruk innsendt turnering om den har rangering, ellers hent fra Firestore
  let t = turnering;
  if (!t?.rangering?.length) {
    t = await hentTurnering(_aktivTurneringId ?? turnering?.id);
  }
  app.aktivTurnering = t;
  ...
  // Bruk lagret rangering om tilgjengelig, ellers beregn på nytt
  let rangering = t.rangering ?? [];
  if (!rangering.length) {
    rangering = beregnEndeligRangering(t);
  }

  // Turneringen er ikke avsluttet — beregn midlertidig rangering fra sluttspill
  ...
  // Støtt både gammelt format { plass, lag: {...} } og nytt { plass, lagId, navn }
