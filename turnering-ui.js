...
// Naviger fremover og push forrige skjerm til stacken
function _navigerFremover(fraSkjerm, tilSkjerm) {
  if (fraSkjerm) _navStack.push(fraSkjerm);
  _naviger(tilSkjerm);
}

// Tilbake-knapp: pop stacken og naviger til forrige skjerm
window.turneringTilbake = async function() {
  ...
  _naviger(forrige);
  // Gjenoppbygg riktig skjerm
  const t = app.aktivTurnering;
  if (!t) return;
  if (forrige === 'turnering')          await visTurneringOversikt();
  if (forrige === 'turnering-oppsett')  visOppsett(t);
  if (forrige === 'turnering-pulje')    visPulje(t);
  if (forrige === 'turnering-bracket')  visBracket(t);
  if (forrige === 'turnering-resultat') visResultat(t);
};
