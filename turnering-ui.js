...
window.leggTilLagUI = async function() {
  ...
  try {
    ...
    oppdaterBracketSetup(t);
    oppdaterOppsettKnapper(t);
    // Fokus settes etter DOM-oppdatering så re-rendering ikke stjeler det tilbake
    inp.focus();
  } catch (e) {
...
// ════════════════════════════════════════════════════════
// RESULTAT-MODAL med klikkbare tall
// ════════════════════════════════════════════════════════
let _tPoeng1   = null;
let _tPoeng2   = null;
let _tAktivLag = 1;
...
window.apneResultatModal = function(puljeEllerNivaa, kampId, lag1Id, lag2Id, erSluttspill = false) {
  ...
  document.getElementById('modal-resultat-format').textContent =
    `${typeLabel}Til ${format.points_to_win} · maks ${format.max_points}`;
...
function _tSettAktivLagUI(lag) {
  _tAktivLag = lag;
  const l1 = document.getElementById('modal-resultat-lag1')?.textContent ?? 'Lag 1';
  const l2 = document.getElementById('modal-resultat-lag2')?.textContent ?? 'Lag 2';
  const el1 = document.getElementById('t-aktiv-lag1');
  const el2 = document.getElementById('t-aktiv-lag2');
  if (el1) { el1.style.border = lag===1?'2px solid var(--accent)':'1px solid var(--border2)'; el1.style.color = lag===1?'var(--accent)':'var(--muted2)'; el1.textContent = lag===1?`${l1} ✎`:l1; }
  if (el2) { el2.style.border = lag===2?'2px solid var(--accent)':'1px solid var(--border2)'; el2.style.color = lag===2?'var(--accent)':'var(--muted2)'; el2.textContent = lag===2?`${l2} ✎`:l2; }
  _tFremhevValgte();
}

function _tOppdaterVis() {
  const v1 = document.getElementById('t-poeng-vis-1');
  const v2 = document.getElementById('t-poeng-vis-2');
  if (v1) { v1.textContent = _tPoeng1 !== null ? _tPoeng1 : '—'; v1.style.color = _tPoeng1 !== null ? 'var(--white)' : 'var(--muted)'; }
  if (v2) { v2.textContent = _tPoeng2 !== null ? _tPoeng2 : '—'; v2.style.color = _tPoeng2 !== null ? 'var(--white)' : 'var(--muted)'; }
}

function _tBygTallgrid(maks) {
  const grid = document.getElementById('t-tall-grid');
  if (!grid) return;
  grid.innerHTML = Array.from({ length: maks + 1 }, (_, i) => i).map(n =>
    `<button onclick="tVelgPoeng(${n})" id="t-tall-${n}"
      style="padding:10px 4px;border-radius:8px;border:1px solid var(--border2);
             background:var(--navy3);color:var(--white);font-size:17px;
             font-family:'Bebas Neue',cursive;cursor:pointer;
             -webkit-tap-highlight-color:transparent">${n}</button>`
  ).join('');
}

function _tFremhevValgte() {
  document.querySelectorAll('[id^="t-tall-"]').forEach(b => {
    b.style.background  = 'var(--navy3)';
    b.style.color       = 'var(--white)';
    b.style.borderColor = 'var(--border2)';
  });
  if (_tPoeng1 !== null && _tAktivLag !== 1) {
    const b = document.getElementById(`t-tall-${_tPoeng1}`);
    if (b) { b.style.background = 'rgba(59,130,246,.2)'; b.style.borderColor = 'var(--accent)'; b.style.color = 'var(--accent2)'; }
  }
  if (_tPoeng2 !== null && _tAktivLag !== 2) {
    const b = document.getElementById(`t-tall-${_tPoeng2}`);
    if (b) { b.style.background = 'rgba(59,130,246,.2)'; b.style.borderColor = 'var(--accent)'; b.style.color = 'var(--accent2)'; }
  }
}

window.tVelgPoeng = function(verdi) {
  if (_tAktivLag === 1) { _tPoeng1 = verdi; if (_tPoeng2 === null) _tSettAktivLagUI(2); }
  else                  { _tPoeng2 = verdi; }
  _tOppdaterVis();
  document.getElementById('modal-resultat-feil').textContent = '';
  _tFremhevValgte();
...
window.bekreftResultat = async function() {
  ...
  if (!val.ok) { document.getElementById('modal-resultat-feil').textContent = val.feil; return; }
  ...
  try {
    if (_modalNivaa) {
      await registrerSluttspillResultat(_aktivTurneringId, _modalNivaa, _modalKampId, p1, p2);
      lukkResultatModal();
      const oppdatert = await hentTurnering(_aktivTurneringId);
      app.aktivTurnering = oppdatert;
      ...
      <div class="kort-innhold" style="padding:10px 12px">
        ...
        ${kamp.ferdig
          ? `<div style="display:flex;justify-content:flex-end;margin-top:8px">
               <button class="t-rediger-knapp" onclick="event.stopPropagation();redigerSluttspillKamp('${escHtml(nivaa)}','${escHtml(kamp.id)}')" title="Rediger resultat">✏️</button>
             </div>`
          : `<div style="text-align:center;margin-top:8px;font-size:13px;color:${klikkbar ? 'var(--accent2)' : 'var(--muted)'}">${klikkbar ? 'Trykk for å registrere' : 'Venter på lag'}</div>`}
      </div>
    </div>`;
}
...
// ════════════════════════════════════════════════════════
// REDIGER SLUTTSPILLRESULTAT
// ════════════════════════════════════════════════════════
let _redigerNivaa  = null;
let _redigerKampId = null;
...
window.redigerSluttspillKamp = function(nivaa, kampId) {
  ...
  // Finn alle nedstrøms kamper som må nullstilles
  const nedstrøms = _finnNedstrøms(kampId, kamper);

  _redigerNivaa  = nivaa;
  _redigerKampId = kampId;

  if (nedstrøms.length === 0) {
    // Ingen nedstrøms — rediger direkte uten advarsel
    apneResultatModal(nivaa, kampId, kamp.lag1Id, kamp.lag2Id, true);
    return;
  }

  // Vis advarsel med antall kamper som nullstilles
  const lagMap = Object.fromEntries((t.lag ?? []).map(l => [l.id, l]));
  const navnListe = nedstrøms.map(k => {
    const l1 = lagMap[k.lag1Id]?.navn ?? '?';
    const l2 = lagMap[k.lag2Id]?.navn ?? '?';
    return `${l1} vs ${l2}`;
  });

  document.getElementById('rediger-advarsel-tekst').innerHTML =
    `Redigering av dette resultatet vil nullstille <strong>${nedstrøms.length}</strong> etterfølgende kamp${nedstrøms.length > 1 ? 'er' : ''}:<br><br>` +
    navnListe.map(n => `• ${escHtml(n)}`).join('<br>');

  document.getElementById('modal-rediger-advarsel').style.display = 'flex';
};

window.lukkRedigerAdvarsel = function() {
  document.getElementById('modal-rediger-advarsel').style.display = 'none';
};

window.bekreftRediger = async function() {
  const t      = app.aktivTurnering;
  const kamper = t?.sluttspill?.[_redigerNivaa]?.kamper ?? [];
  const kamp   = kamper.find(k => k.id === _redigerKampId);
  if (!kamp) return;

  lukkRedigerAdvarsel();

  const knapp = document.querySelector('#modal-rediger-advarsel .knapp-fare');
  try {
    // Nullstill nedstrøms kamper i Firestore
    await nullstillNedstrømsKamper(_aktivTurneringId, _redigerNivaa, _redigerKampId);
    // Hent oppdatert turnering og åpne resultat-modal
    const oppdatert = await hentTurnering(_aktivTurneringId);
    app.aktivTurnering = oppdatert;
    visBracket(oppdatert);
    // Åpne modal for ny registrering
    apneResultatModal(_redigerNivaa, _redigerKampId, kamp.lag1Id, kamp.lag2Id, true);
  } catch (e) {
    visMelding(e?.message ?? 'Feil ved nullstilling.', 'feil');
  }
};

// Finn alle kamper nedstrøms fra en gitt kamp (rekursivt)
function _finnNedstrøms(kampId, alleKamper) {
  ...
  const besøk = (id) => {
    const k = alleKamper.find(k => k.id === id);
    if (!k || !k.ferdig) return;
    resultat.push(k);
    if (k.winner_to) besøk(k.winner_to);
    if (k.loser_to)  besøk(k.loser_to);
  };

  if (kamp.winner_to) besøk(kamp.winner_to);
  if (kamp.loser_to)  besøk(kamp.loser_to);
  return resultat;
}
