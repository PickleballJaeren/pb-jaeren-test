...
export async function visTurneringOversikt() {
  ...
  try {
    ...
    if (!turneringer.length) {
      container.innerHTML = `
        <div class="t-tom-tilstand">
          <div class="t-tom-ikon">🏆</div>
          <div class="t-tom-tittel">Ingen turneringer ennå</div>
          <div class="t-tom-sub">Opprett en ny for å komme i gang</div>
        </div>`;
      return;
    }
...
function oppdaterLagListe(t) {
  ...
  if (!t.lag?.length) {
    liste.innerHTML = `<div class="tom-tilstand">Ingen lag lagt til ennå</div>`;
    return;
  }
...
function oppdaterPuljePreview(t) {
  ...
  if (t.lag.length < antall * 2) {
    container.innerHTML = `<div class="tom-tilstand-liten">Legg til minst ${antall * 2} lag for ${antall} puljer</div>`;
    return;
  }
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${p.lagIds.map((id, i) => `
            <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--navy3);border:1px solid var(--border);border-radius:8px;font-size:16px">
              <span class="t-lag-seed">${i + 1}</span>
              <span>${escHtml(lagMap[id]?.navn ?? id)}</span>
            </div>`).join('')}
        </div>
      </div>`).join('');
  } catch (e) {
    container.innerHTML = `<div class="tom-tilstand-liten">${escHtml(e.message)}</div>`;
  }
}
