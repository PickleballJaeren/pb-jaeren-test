...
async function lastSpillerListe() {
  ...
  // Bevar scrollposisjonen så listen ikke hopper til toppen etter re-render
  const scrollBeholder = container.closest('.scroll-omrade') ?? container.parentElement;
  const scrollPos      = scrollBeholder?.scrollTop ?? 0;
  ...
  try {
    ...
    if (scrollBeholder) scrollBeholder.scrollTop = scrollPos;
  } catch (e) {
