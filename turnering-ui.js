...
window.nullstillKampformat = async function() {
  ...
  try {
    const t = app.aktivTurnering;
    const erSetupFase = t?.status === T_STATUS.SETUP || t?.status === T_STATUS.GROUP_PLAY || t?.status === T_STATUS.PLAYOFF_SEEDING;

    if (erSetupFase) {
      // Oppsettfase — nullstill i konfig
      const konfigNøkkel = `kampformat${_kfNivaa === 'A' ? 'Kvartfinale' : 'Pulje'}`;
      await oppdaterTurneringKonfig(_aktivTurneringId, { [konfigNøkkel]: null });
    } else {
      // Live-fase — nullstill på kamp-nivå
      await oppdaterKampformat(_aktivTurneringId, _kfNivaa, _kfKampId, null);
    }
    lukkKampformatModal();
    const oppdatert = await hentTurnering(_aktivTurneringId);
    app.aktivTurnering = oppdatert;
    oppdaterBracketSetup(oppdatert);
...
window.bekreftKampformat = async function() {
  ...
  try {
    ...
    if (erSetupFase) {
      // Oppsettfase — lagre i konfig basert på runde-type
      const rundeNøkkel = {
        'Kvartfinale':    'kampformatKvartfinale',
        'Plass 5–8':      'kampformatKvartfinale',
        'Semifinale':     'kampformatSemifinale',
        '1. plass':       'kampformatFinale',
        '3. plass':       'kampformatFinale',
        '5. plass':       'kampformatFinale',
        '7. plass':       'kampformatFinale',
        '9. plass':       'kampformatFinale',
        '17. plass':      'kampformatFinale',
      }[_kfRunde] ?? 'kampformatKvartfinale';
      await oppdaterTurneringKonfig(_aktivTurneringId, { [rundeNøkkel]: format });
    } else {
      // Live-fase — lagre på kamp-nivå
...
window.leggTilLagUI = async function() {
  ...
  try {
    // Tøm feltet og sett fokus synkront — før await — så iOS ikke mister
    // brukertillatelsen og tastaturet forblir oppe på iPad/iPhone.
    inp.value = '';
    inp.focus();
