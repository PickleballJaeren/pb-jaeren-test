...
/**
 * Genererer B/C-bracket med plasseringskamper.
 *
 * Støttede størrelser:
 *   2 lag  → finale
 *   3–4 lag → semifinale + finale + bronsekamp
 *   5–6 lag → kvartfinale (med bye) + semifinale + finale + bronsekamp
 *   7 lag   → bye til seed 1, 3 kvartfinaler + semifinale + finale + bronsekamp
 *   8 lag   → 4 kvartfinaler + semifinale + finale + bronsekamp
 */
export function genererBCBracket(lagIds, nivaa, startPlass) {
  ...
  if (n <= 4) {
    const s = lagIds;
    kamper.push(_lagKamp(`${prefix}_SF1`, s[0], s[3] ?? null, `${prefix}_FIN`, `${prefix}_BRO`, 'Semifinale'));
    kamper.push(_lagKamp(`${prefix}_SF2`, s[1], s[2] ?? null, `${prefix}_FIN`, `${prefix}_BRO`, 'Semifinale'));
  ...
  // 7 lag — seed 1 får bye direkte til semifinale
  // QF: seed2 vs seed7, seed3 vs seed6, seed4 vs seed5
  // SF: seed1 (bye) vs QF1-vinner, QF2-vinner vs QF3-vinner
  if (n === 7) {
    const s = lagIds;
    ...
    // SF1: seed1 (bye) venter på QF1-vinner
    kamper.push(_lagKamp(`${prefix}_SF1`, s[0], null, `${prefix}_FIN`, `${prefix}_BRO`, 'Semifinale'));
    kamper.push(_lagKamp(`${prefix}_SF2`, null,  null, `${prefix}_FIN`, `${prefix}_BRO`, 'Semifinale'));
    kamper.push(_lagKamp(`${prefix}_FIN`, null,  null, null, null, `${startPlass}. plass`));
    kamper.push(_lagKamp(`${prefix}_BRO`, null,  null, null, null, `${bronse}. plass`));
    return kamper;
  }

  // 5–6 og 8 lag — standard kvartfinale med eventuelle byer for null-slots
  // Seeding: 1v8, 4v5, 2v7, 3v6 (8-lag standard)
  const s = lagIds;
  ...
  kamper.push(_lagKamp(`${prefix}_SF1`, null, null, `${prefix}_FIN`, `${prefix}_BRO`, 'Semifinale'));
  kamper.push(_lagKamp(`${prefix}_SF2`, null, null, `${prefix}_FIN`, `${prefix}_BRO`, 'Semifinale'));
  kamper.push(_lagKamp(`${prefix}_FIN`, null, null, null, null, `${startPlass}. plass`));
  kamper.push(_lagKamp(`${prefix}_BRO`, null, null, null, null, `${bronse}. plass`));
  return kamper;
...
export function beregnEndeligRangering(turnering) {
  ...
  if (sluttspill?.B?.kamper?.length) {
    ...
    // Kvartfinaletapere: seed 1 har bye i QF ved 7 lag — hent kun ferdigspilte QF-kamper
