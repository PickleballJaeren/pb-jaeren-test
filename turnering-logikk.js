...
export function genererABracket(seededeIds, konfig, parOverstyr = null) {
  ...
  // 7 lag: seed 1 får bye til SF1, seed 2 vs 7 / seed 3 vs 6 / seed 4 vs 5 i QF
  if (n === 7) {
    ...
    kamper.push(_lagKamp('A_QF3', seededeIds[3], seededeIds[4], 'A_SF2', plasseringPaa ? 'A_P5_SF2' : null, 'Kvartfinale'));
...
function _kryssSeeding2Puljer(meta, puljer) {
  ...
  // Hvis puljene har ulikt antall lag, stryk resultater mot det svakeste
  // laget i den største puljen slik at alle sammenlignbare lag har spilt
  // like mange kamper. Standard praksis i turneringer med ujevne puljer.
  const justertStats = _justerForUjevnePuljer(p0, p1, puljer);

  // Kryss-seeding med sammenligning på tvers:
  // For hvert rang-par (A1 vs B1, A2 vs B2 osv.) settes det beste laget
  // som odde seed (1, 3, 5...) og det svakeste som påfølgende seed.
...
/**
 * Beregner justerte statistikker for lag i den største puljen ved å
 * stryke kampen mot det svakeste laget (siste i tabellen).
 * Returnerer et map { lagId → justert statistikk } for de aktuelle lagene.
 * Lag i den minste puljen returneres uendret.
 */
function _justerForUjevnePuljer(p0, p1, puljer) {
  const justertStats = {};

  // Finn hvilken pulje som er størst
  const storPulje  = p0.length > p1.length ? p0 : (p1.length > p0.length ? p1 : null);
  if (!storPulje) return justertStats; // like store — ingen justering

  const svakeste  = storPulje[storPulje.length - 1]?.lagId;
  if (!svakeste) return justertStats;

  // Finn alle kamper i den store puljen
  const storPuljeObj = puljer.find(p =>
    p.lagIds?.includes(svakeste)
  );
  if (!storPuljeObj) return justertStats;

  // For hvert lag i stor pulje: trekk fra resultatet mot svakeste lag
  for (const lagMeta of storPulje) {
    const id = lagMeta.lagId;
    const kampMotSvakeste = (storPuljeObj.kamper ?? []).find(k =>
      k.ferdig &&
      ((k.lag1Id === id && k.lag2Id === svakeste) ||
       (k.lag2Id === id && k.lag1Id === svakeste))
    );

    if (!kampMotSvakeste) {
      // Ingen kamp funnet — bruk original statistikk
      justertStats[id] = lagMeta;
      continue;
    }

    const erLag1  = kampMotSvakeste.lag1Id === id;
    const mineP   = erLag1 ? kampMotSvakeste.lag1Poeng : kampMotSvakeste.lag2Poeng;
    const deresP  = erLag1 ? kampMotSvakeste.lag2Poeng : kampMotSvakeste.lag1Poeng;
    const vant    = mineP > deresP;

    justertStats[id] = {
      ...lagMeta,
      seire:  lagMeta.seire  - (vant ? 1 : 0),
      pf:     lagMeta.pf     - mineP,
