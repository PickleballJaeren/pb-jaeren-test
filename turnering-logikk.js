...
export function beregnPuljetabell(pulje, alleLag) {
  ...
  const sortert = _sorterPuljeTabell(Object.values(stats), pulje.kamper ?? []);
  return sortert;
}

/**
 * Sorterer og annoterer tabellen med tiebreakType per rad.
 * tiebreakType: null | 'innbyrdes' | 'poengdiff'
 * Brukes av UI til å vise farget venstrekant.
 */
...
/**
 * Sorterer en gruppe av lag med like seire og setter tiebreakType.
 *
 * Rekkefølge:
 *   1. Innbyrdes seire (kun kamper mellom lagene i gruppen)
 *   2. Ved sirkel (alle likt innbyrdes): total poengdifferanse
 *   3. Total scorede poeng
 */
function _sorterGruppeInnbyrdes(gruppe, kamper) {
  const ids  = new Set(gruppe.map(l => l.lagId));
  const mini = {};
  gruppe.forEach(l => { mini[l.lagId] = { seire: 0 }; });
  ...
  const totaler       = gruppe.map(l => mini[l.lagId].seire);
  const erSirkel      = totaler.every(s => s === totaler[0]);
  const tiebreakType  = erSirkel ? 'poengdiff' : 'innbyrdes';

  const sortert = [...gruppe].sort((a, b) => {
    const innS = mini[b.lagId].seire - mini[a.lagId].seire;
    if (!erSirkel && innS !== 0) return innS;
    if (b.pd !== a.pd) return b.pd - a.pd;
    return b.pf - a.pf;
  });

  sortert.forEach(l => { l.tiebreakType = tiebreakType; });
  return sortert;
}
