...
function _sorterPuljeTabell(lagListe, kamper) {
  // Trinn 1: grovsorter på totale seire
  const grov = [...lagListe].sort((a, b) => {
    if (b.seire !== a.seire) return b.seire - a.seire;
    if (b.pd    !== a.pd)    return b.pd    - a.pd;
    return b.pf - a.pf;
  });

  // Trinn 2: finn grupper med helt like seire og løs dem med innbyrdes mini-tabell
  const resultat = [];
  let i = 0;
  while (i < grov.length) {
    let j = i + 1;
    ...
    if (gruppe.length === 1) {
      ...
      resultat.push(..._sorterGruppeInnbyrdes(gruppe, kamper));
    }
    i = j;
  }
  return resultat;
...
/**
 * Sorterer en gruppe av lag med like seire ved å beregne en
 * mini-tabell kun over innbyrdes kamper.
 * Rekkefølge: innbyrdes seire → innbyrdes pd → innbyrdes pf → total pd → total pf
 */
function _sorterGruppeInnbyrdes(gruppe, kamper) {
  const ids  = new Set(gruppe.map(l => l.lagId));
  const mini = {};
  gruppe.forEach(l => { mini[l.lagId] = { seire: 0, pf: 0, pm: 0 }; });

  for (const k of kamper) {
    if (!k.ferdig || k.lag1Poeng == null || k.lag2Poeng == null) continue;
    if (!ids.has(k.lag1Id) || !ids.has(k.lag2Id)) continue;
    mini[k.lag1Id].pf += k.lag1Poeng; mini[k.lag1Id].pm += k.lag2Poeng;
    mini[k.lag2Id].pf += k.lag2Poeng; mini[k.lag2Id].pm += k.lag1Poeng;
    if (k.lag1Poeng > k.lag2Poeng) mini[k.lag1Id].seire++;
    else                            mini[k.lag2Id].seire++;
  }

  return [...gruppe].sort((a, b) => {
    const ma = mini[a.lagId], mb = mini[b.lagId];
    const innS  = mb.seire - ma.seire;           if (innS  !== 0) return innS;
    const innPd = (mb.pf - mb.pm) - (ma.pf - ma.pm); if (innPd !== 0) return innPd;
    const innPf = mb.pf - ma.pf;                 if (innPf !== 0) return innPf;
    // Fortsatt likt — fall tilbake på total pd og pf
    if (b.pd !== a.pd) return b.pd - a.pd;
    return b.pf - a.pf;
  });
}
...
/** Genererer B/C-bracket (kun vinnerbane). */
export function genererBCBracket(lagIds, nivaa, startPlass) {
  ...
  if (nivaaStart === 'finale') {
    kamper.push(_lagKamp(`${prefix}_FIN`, lagIds[0], lagIds[1] ?? null, null, null, `${startPlass}. plass`));
    return kamper;
  }

  if (nivaaStart === 'semifinale') {
    const s = lagIds;
    kamper.push(_lagKamp(`${prefix}_SF1`, s[0], s[3] ?? null, `${prefix}_FIN`, null, 'Semifinale'));
    kamper.push(_lagKamp(`${prefix}_SF2`, s[1], s[2] ?? null, `${prefix}_FIN`, null, 'Semifinale'));
  ...
  if (nivaaStart === 'kvartfinale') {
    const s = lagIds;
    kamper.push(_lagKamp(`${prefix}_QF1`, s[0], s[7] ?? null, `${prefix}_SF1`, null, 'Kvartfinale'));
    kamper.push(_lagKamp(`${prefix}_QF2`, s[3], s[4] ?? null, `${prefix}_SF1`, null, 'Kvartfinale'));
    kamper.push(_lagKamp(`${prefix}_QF3`, s[1], s[6] ?? null, `${prefix}_SF2`, null, 'Kvartfinale'));
    kamper.push(_lagKamp(`${prefix}_QF4`, s[2], s[5] ?? null, `${prefix}_SF2`, null, 'Kvartfinale'));
    kamper.push(_lagKamp(`${prefix}_SF1`, null, null, `${prefix}_FIN`, null, 'Semifinale'));
    kamper.push(_lagKamp(`${prefix}_SF2`, null, null, `${prefix}_FIN`, null, 'Semifinale'));
    kamper.push(_lagKamp(`${prefix}_FIN`, null, null, null, null, `${startPlass}. plass`));
    return kamper;
  }
  ...
  const s = lagIds;
  for (let i = 0; i < 8; i++) {
    kamper.push(_lagKamp(`${prefix}_R1_${i+1}`, s[i] ?? null, s[15-i] ?? null, `${prefix}_QF${Math.floor(i/2)+1}`, null, 'Åttedelsfinale'));
  }
  kamper.push(_lagKamp(`${prefix}_QF1`, null, null, `${prefix}_SF1`, null, 'Kvartfinale'));
  kamper.push(_lagKamp(`${prefix}_QF2`, null, null, `${prefix}_SF1`, null, 'Kvartfinale'));
  kamper.push(_lagKamp(`${prefix}_QF3`, null, null, `${prefix}_SF2`, null, 'Kvartfinale'));
  kamper.push(_lagKamp(`${prefix}_QF4`, null, null, `${prefix}_SF2`, null, 'Kvartfinale'));
  kamper.push(_lagKamp(`${prefix}_SF1`, null, null, `${prefix}_FIN`, null, 'Semifinale'));
  kamper.push(_lagKamp(`${prefix}_SF2`, null, null, `${prefix}_FIN`, null, 'Semifinale'));
  kamper.push(_lagKamp(`${prefix}_FIN`, null, null, null, null, `${startPlass}. plass`));
  return kamper;
