...
export function genererABracket(seededeIds, konfig, parOverstyr = null) {
  ...
  if (n === 6) {
    kamper.push(_lagKamp('A_QF1', seededeIds[2], seededeIds[5], 'A_SF1', plasseringPaa ? 'A_P5_SF1' : null, 'Kvartfinale'));
    kamper.push(_lagKamp('A_QF2', seededeIds[3], seededeIds[4], 'A_SF2', plasseringPaa ? 'A_P5_SF2' : null, 'Kvartfinale'));
    ...
    if (plasseringPaa) {
      kamper.push(_lagKamp('A_BRO', null, null, null, null, '3. plass'));
      kamper.push(_lagKamp('A_P5_SF1', null, null, 'A_P5_FIN', null, 'Plass 5–8'));
      kamper.push(_lagKamp('A_P5_SF2', null, null, 'A_P5_FIN', null, 'Plass 5–8'));
      kamper.push(_lagKamp('A_P5_FIN', null, null, null, null, '5. plass'));
    }
    return kamper;
  }
...
function _kryssSeeding2Puljer(meta, puljer) {
  ...
  // Kryss-seeding: alternerer mellom puljene, rang for rang.
  // Seed 1=A1, 2=B1, 3=A2, 4=B2, 5=A3, 6=B3, 7=A4, 8=B4
  // Dette gir riktig bye-fordeling uansett antall lag (4–8).
