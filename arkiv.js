// ════════════════════════════════════════════════════════
// arkiv.js — Økt-arkiv og øktsletting
// Håndterer listing av tidligere økter, visning av detaljer
// for enkeltøkter, samt sletting av enkeltøkter eller alle.
//
// Avhengigheter som må registreres av app.js ved oppstart:
//   - registrerGetAktivKlubbId(() => aktivKlubbId)
//   - registrerKrevAdmin(krevAdminMedDemo)
// ════════════════════════════════════════════════════════
...
import { visMelding, visFBFeil, escHtml, naviger } from './ui.js';

// ════════════════════════════════════════════════════════
// KOBLINGER TIL APP.JS — settes ved oppstart
// ════════════════════════════════════════════════════════
let _getAktivKlubbId = () => null;
let _krevAdmin       = (_tittel, _tekst, cb) => cb(); // fallback: alltid admin

/**
 * Registrerer en funksjon som returnerer ID-en til aktiv klubb.
 * Kalles fra app.js ved oppstart: registrerGetAktivKlubbId(() => aktivKlubbId).
 */
export function registrerGetAktivKlubbId(fn) {
  _getAktivKlubbId = fn;
}

/**
 * Registrerer krevAdmin-funksjonen fra app.js (som inkluderer demo-flagget).
 * Kalles fra app.js ved oppstart: registrerKrevAdmin(krevAdminMedDemo).
 */
export function registrerKrevAdmin(fn) {
  _krevAdmin = fn;
}
...
// INTERN TILSTAND
// ════════════════════════════════════════════════════════
// ID-en til økten som vises i detaljvisning akkurat nå.
// Brukes av slett-modalen til å vite hvilken økt som skal slettes.
let aktivTreningDetaljId = null;

// ════════════════════════════════════════════════════════
// LAST ARKIV — liste over alle økter for aktiv klubb
// ════════════════════════════════════════════════════════
