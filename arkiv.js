...
window.apneTurneringArkivDetalj = async function(turneringId) {
  ...
  try {
    ...
    const antallLag    = (t.lag ?? []).length;
    document.getElementById('t-arkiv-meta').innerHTML = [
      { ikon: '🏐', tekst: antallLag + ' lag' },
      { ikon: '🔢', tekst: antallPuljer + ' puljer' },
    ].map(c => `<div style="display:inline-flex;align-items:center;gap:5px;padding:5px 10px;background:var(--navy3);border:1px solid var(--border);border-radius:20px;font-size:15px;color:var(--muted2)">
  ...
  for (const p of puljer) {
    ...
    for (const rNr of Object.keys(runder).sort((a,b) => a-b)) {
      ...
      for (const k of runder[rNr]) {
        ...
        const v1 = k.lag1Poeng > k.lag2Poeng;
        html += `<div class="kamp-rad" style="margin-bottom:6px">
          <div style="flex:1">
            <div style="font-size:16px;font-weight:${v1?600:400};color:${v1?'var(--white)':'var(--muted2)'}">${escHtml(l1)}</div>
            <div style="font-size:16px;font-weight:${!v1?600:400};color:${!v1?'var(--white)':'var(--muted2)'}">${escHtml(l2)}</div>
          </div>
          <div style="font-family:'DM Mono',monospace;font-size:20px;color:var(--green2);display:flex;flex-direction:column;align-items:flex-end;gap:2px">
            <span>${k.lag1Poeng}</span><span>${k.lag2Poeng}</span>
          </div>
        </div>`;
      }
    }
  ...
  const renderBracket = (nivaa, kamper, farge) => {
    ...
    for (const [rundeNavn, kampListe] of sorterte) {
      ...
      for (const k of kampListe) {
        ...
        const v1 = k.lag1Poeng > k.lag2Poeng;
        bHtml += `<div class="kamp-rad" style="margin-bottom:6px">
          <div style="flex:1">
            <div style="font-size:16px;font-weight:${v1?600:400};color:${v1?'var(--white)':'var(--muted2)'}">${escHtml(l1)}</div>
            <div style="font-size:16px;font-weight:${!v1?600:400};color:${!v1?'var(--white)':'var(--muted2)'}">${escHtml(l2)}</div>
          </div>
